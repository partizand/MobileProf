Функция ВыборВопроса(Очередь = "next", ИгнорироватьФлагТестЗавершен = Ложь)
	ТекТаблица = Неправильные;
	
	Если ТестЗавершен Тогда
		ТекТаблица = Отвеченные;
	КонецЕсли;
	...
	//Если ТестЗавершен и НЕ ИгнорироватьФлагТестЗавершен Тогда
		...
		Первый_Ответ = ТекТаблица[0];
		Последний_Ответ = ТекТаблица[ТекТаблица.Количество() - 1];
		
		НайденныеСтроки = ТекТаблица.НайтиСтроки(Новый Структура("Вопрос", ТекущийВопрос));
		...
		ИначеЕсли Очередь = "next" Тогда
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				ТекОчередь = "first";
			Иначе
				ТекИндекс = ТекТаблица.Индекс(НайденныеСтроки[0]);
				Если (ТекИндекс + 2) > ТекТаблица.Количество() Тогда
					ТекИндекс = 0;
					ТекОчередь = "first";
				Иначе
				...
				Следующий_Ответ = ТекТаблица.Получить(ТекИндекс);
		...
		ИначеЕсли Очередь = "prev" Тогда
			...
			Иначе
				...
				Если (ТекИндекс = 0) Тогда
					ТекИндекс = ТекТаблица.Количество() - 1;
					ТекОчередь = "last";
				Иначе
					ТекИндекс = ТекИндекс - 1;
					ТекОчередь = "prev";
				...
				Предыдущий_Ответ = ТекТаблица.Получить(ТекИндекс);
			КонецЕсли;
...
Процедура ПоказатьВопрос(СсылкаВопрос)
	...
	Комментарий = ВыборкаВопрос.Комментарий;
	...
	Элементы.Комментарий.Видимость = ТестЗавершен И ((НЕ ПустаяСтрока(Комментарий)));
...
Функция КомандаВыборВопросаНаСервере()
	...
	Если ТестЗавершен Тогда
		СписокСсылок.ЗагрузитьЗначения(Отвеченные.Выгрузить().ВыгрузитьКолонку("Вопрос"));
	Иначе
		СписокСсылок.ЗагрузитьЗначения(Неправильные.Выгрузить().ВыгрузитьКолонку("Вопрос"));
	КонецЕсли;                                  
...
Процедура КомандаВыборВопроса(Команда)
	//Если ТестЗавершен Тогда
		ОткрытьФорму("Справочник.ВопросыИОтветы.ФормаВыбора", Новый Структура("ТекущаяСтрока, Ссылки", ТекущийВопрос, КомандаВыборВопросаНаСервере()),ЭтаФорма);
	//Иначе
	//	ОткрытьФорму("Справочник.ВопросыИОтветы.ФормаВыбора", Новый Структура("ТекущаяСтрока", ТекущийВопрос),ЭтаФорма);
	//КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НеПоказыватьУдаленные", РегистрыСведений.ХранилищеНастроек.ПолучитьНастройку("НеПоказыватьУдаленные", Истина));
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	РезультатыТестированияСрезПоследних.Вопрос,
	               |	РезультатыТестированияСрезПоследних.Вопрос.НомерВопроса КАК НомерВопроса,
	               |	РезультатыТестированияСрезПоследних.Вопрос.Владелец.НомерРаздела КАК НомерРаздела
	               |ИЗ
	               ...
	               |	И НЕ(РезультатыТестированияСрезПоследних.Вопрос.ПометкаУдаления
	               |				И &НеПоказыватьУдаленные)
	               |	И НЕ(РезультатыТестированияСрезПоследних.Вопрос.Владелец.ПометкаУдаления
	               |				И &НеПоказыватьУдаленные)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	НомерРаздела,
	...
	Если Выборка.Количество() = 0 Тогда
		ЗакрытьПриОткрытии = Истина;
		Сообщить("Нет вопросов!");
		Возврат;
	КонецЕсли;
	
	Пока Выборка.Следующий() Цикл
		НовСтр = Неправильные.Добавить();
		НовСтр.Вопрос = Выборка.Вопрос;
	КонецЦикла;
...
&НаСервере
Процедура КомандаОтветитьНаСервере(ПереходитьНаСледующий = Истина)
